...
#to get the extent of one of these:
#ext(elc_habitat)
...
writeRaster(deer_w, here::here("Output", "deer_w2.tiff"), overwrite = TRUE)
...
disthumaccess2<-resample(disthumanaccess, mask.raster, method="bilinear")
...
#remove unused NAME levels (seemingly unnecessary when they are characters, however,
#if they are factors then would have to potentially use the code below)
#an atomic vector is like a list of lists
...
gridded(hr95) <- TRUE 
## display the results 
#could convert this into a mask if you wanted
image(hr95)
```
...
disthumanaccess <- resample(disthumanaccess, deer_w)
disthighhumanaccess <- resample(disthighhumanaccess, deer_w)

#stacking rasters
all_rasters<-c(deer_w, moose_w, elk_w, sheep_w, goat_w, wolf_w,elevation2, disthumanaccess, disthighhumanaccess)
...
crs(elevation2) <- "+proj=utm +zone=11 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0"
crs(disthumanaccess) <- "+proj=utm +zone=11 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0"
crs(disthighhumanaccess) <- "+proj=utm +zone=11 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0"
class(all_rasters)

#Extract covariate values for Red Deer wolf data  
cov.outRD<- raster::extract(all_rasters, rd.data)
head(cov.outRD)

#Extract covariate values for available points
rd.avail_sf<-st_as_sf(rd.avail) #first change rd.avail to data type sf
cov.availRD<-raster::extract(all_rasters, rd.avail_sf)

#Extract covariate values for Bow valley wolf data  
cov.outBV<-raster::extract(all_rasters, bv.data)

#Extract covariate values for available points
bv.avail_sf <- st_as_sf(bv.avail)
cov.availBV<-raster::extract(all_rasters, bv.avail_sf)
```
